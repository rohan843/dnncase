...
Clears the state. Use it if new models will be repeatedly created from the same code in a loop, to save memory.

### epsilon

Returns a small number that can be used in place of `0` to prevent divide by zero errors.

### floatx

Returns whatever is the default float type: 'float16', 'float32', 'float64' etc.

### get_uid

> Best not to use it.

> It might be a good idea to create a custom fn that does the same job.

Returns an ID for a string. Increments the ID with every call.

### image_data_format

Returns the default image data format convention, either 'channels_first' or 'channels_last'.

### is_keras_tensor

> Seems too technical to be of any use to us.
...
Sets the value of fuzz factor.

### set_floatx

Sets the default float precision.

### set_image_data_format

Sets image data format to 'channels_first' or 'channels_last'.
